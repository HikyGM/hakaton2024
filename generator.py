...
class ProblemGenerator:
    ...
    def add_expander(self, func: object) -> object:
...
def __l_div(num):
    a = int(num) * random.randrange(0, 100)
    if a == 0:
...
def __l_pow(num):
    if num == 0:
        return str(random.randint(2, 3)) + "^{-\\infty}"
...
# gen = generator.ProblemGenerator()
# gen.add_expander(generator.__l_sum)
# gen.add_expander(generator.__l_div)
# gen.add_expander(generator.__l_pow)
# gen.add_expander(generator.__l_sqrt)
# gen.add_expander(generator.__l_int)
# gen.add_expander(generator.__l_sig)

# import matplotlib.pyplot as plt
#
# plt.axis("off")
# latex_expression = gen.randexpr(1, 2)  # 30 раз заменяем. Выражение будет равно 1
